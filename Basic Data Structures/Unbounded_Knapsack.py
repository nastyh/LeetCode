def unbounded_knapsack(profit, weight, capacity):  # O(N*capacity) both, where N = len(profit)
    ...
    for row in range(1, len(profit)):  # starting from cell (1, 1)
        for col in range(1, capacity + 1):
            ...
            else:  # otherwise, choose the max between the previous choice (means not taking current item) and taking the current profit and whatever the profit is from the leftovers above
