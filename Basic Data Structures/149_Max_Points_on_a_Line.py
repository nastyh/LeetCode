 ...
 def maxPoints(points):
    ...
    for i in range(len(points)):
        for j in range(i + 1, len(points)):
            ...
            if a < 0:
                a, b, c = -a, -b, -c
            # Get common divisor to normalize a, b, c
            g = math.gcd(a, b, c)
            line = (a/g, b/g, c/g)
            # Add the counter for the point instead of the coords for better performance
            d[line].add(i)
            d[line].add(j)
    # return the max of the length of points in line
    return max(map(len, d.values()))

 
 def maxPoints(self, points):  # O(N^2) and O(N)
